        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="(item,index) in cartList" :key="'cart-'+index">
...
export default {
  ...
  computed: {
    ...
    totalPrice () {
      ...
      this.cartList.forEach(item => {
        if(item.checked === 1) {
          allprice += parseFloat(item.salePrice) * parseInt(item.productNum)
        }
      })
      return allprice
    }
  ...
  methods: {
    ...
    editCart(flag,item) {
      ...
      this.$http.post('/users/cartEdit',{productId,productNum,checked}).then(res => {
        res = res.data
        if (res.status === '0') {
